...
class NetWrapper(nn.Module):
    ...
    def forward(self, x):
        representation, _ = self.get_representation(x)
...
class BYOL(nn.Module):
    def __init__(
        ...
        self.forward(torch.randn(2, 1, image_size), torch.randn(2, 1, image_size))
    ...
    def forward(self, image_one, image_two):
        ...
        criterion = byol_losses.ByolLoss()

        loss_one = criterion(online_pred_one, target_proj_two.detach())
        loss_two = criterion(online_pred_two, target_proj_one.detach())
