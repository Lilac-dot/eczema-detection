"""
Train a LightGBM classifier (the best-performing model in Maulana et al. 2024) on the
90 hand-engineered colour features extracted in extract_color_features.py, using the
SAME train/val/test split as the ResNet18 model (manifest_train/val/test.csv) so the
...
FEATURES_CSV = ROOT / "color_features.csv"
...
def main():
    ...
    model.save_model(str(Path(r"C:\Users\tishy\Documents\Honors\models\stage_b_lightgbm.txt")))
    print("\nModel saved to models/stage_b_lightgbm.txt")
