"""
Same as train_stage_b_lightgbm.py, but trained on dataset/color_features_cropped.csv
(colour features computed on the auto-cropped images) instead of the original
full-frame colour features. Same train/val/test split membership.
"""
import csv
...
FEATURES_CSV = ROOT / "color_features_cropped.csv"
...
def load_split(split_name, by_path):
    manifest = ROOT / f"manifest_cropped_{split_name}.csv"
    X, y = [], []
...
def main():
    ...
    model.save_model(str(Path(r"C:\Users\tishy\Documents\Honors\models\stage_b_lightgbm_cropped.txt")))
    print("\nModel saved to models/stage_b_lightgbm_cropped.txt")
