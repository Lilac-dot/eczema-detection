"""
Experiment 2c (fine-tune-more, continuation of Experiment 2): identical to
train_curated_cnn_multisource.py (unweighted ConcatDataset, same LR/epochs/checkpoint
criterion), except SCIN-train is manifest_scin_train_expanded.csv (582 images, 291/291)
from expand_scin_train.py instead of the original manifest_scin_train.csv (488, 244/244).
Isolates "more SCIN training data" as the single variable, separate from Experiment 2b's
weighted-sampling variable -- run after that experiment, not concurrently with it, to keep
wall-clock timing comparable on this CPU-only machine.

SCIN val/test are unchanged from Experiment 2's original split -- expand_scin_train.py never
touched them -- so results here are directly comparable to Experiment 2's and Experiment 2b's
held-out numbers.

Saves to models/curated_resnet18_multisource_expanded.pt.
"""
import csv
import random
import time
...
OUT_PATH = MODELS_DIR / "curated_resnet18_multisource_expanded.pt"
...
def main():
    ...
    train_ds = ConcatDataset([
        ...
        ManifestDataset(SCIN_DIR / "manifest_scin_train_expanded.csv", train_tf),
        ManifestDataset(SKINDISNET_DIR / "manifest_skindisnet_train.csv", train_tf),
    ])
    print(f"Combined training set: {len(train_ds)} images (SCIN expanded to 582)")
