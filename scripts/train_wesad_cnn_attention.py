...
Per-fold normalization: each modality is z-scored using ONLY the training fold's mean/std
(computed fresh per fold), never global statistics -- avoids leaking test/val-subject scale
information into training, same spirit as the subject-level split itself.
...
FOLD_RESULTS_CSV = ROOT / "dataset" / "WESAD" / "wesad_cnn_loso_fold_results.csv"
SEED = 42
EMBED_DIM = 32
MAX_EPOCHS = 60
PATIENCE = 10
...
def to_tensors(idx, EDA, TEMP, BVP, ACC, label, eda_mu, eda_sd, temp_mu, temp_sd, bvp_mu, bvp_sd, acc_mu, acc_sd):
    e = torch.tensor((EDA[idx] - eda_mu) / eda_sd, dtype=torch.float32).unsqueeze(1)     # (n,1,120)
    t = torch.tensor((TEMP[idx] - temp_mu) / temp_sd, dtype=torch.float32).unsqueeze(1)  # (n,1,120)
    b = torch.tensor((BVP[idx] - bvp_mu) / bvp_sd, dtype=torch.float32).unsqueeze(1)      # (n,1,1920)
    a = torch.tensor((ACC[idx] - acc_mu) / acc_sd, dtype=torch.float32).permute(0, 2, 1)  # (n,3,960)
...
def train_one_fold(train_idx, val_idx, EDA, TEMP, BVP, ACC, label):
    # fold-local normalization stats, from TRAIN subjects only
...
def main():
    ...
    print(f"\nFinal model saved: models/wesad_stress_cnn_attention.pt")
