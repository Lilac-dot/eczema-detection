"""
Train a LightGBM classifier on the 90 colour features for the curated
Eczema-vs-other-skin-condition subset (SkinDisease/color_features_curated.csv),
using the same dual-model approach as Stage B: this model exists specifically to
cross-check the CNN result, since it caught the original Eczema-vs-Normal shortcut.
If this colour-only model scores anywhere near as well as the CNN here, that's the
same red flag as before.
"""
import csv
...
FEATURES_CSV = ROOT / "color_features_curated.csv"
...
def load_split(split_name, by_path):
    manifest = ROOT / f"manifest_curated_{split_name}.csv"
    X, y = [], []
...
def main():
    ...
    params = {
        ...
        "learning_rate": 0.05,
        "num_leaves": 31,
        "is_unbalance": True,  # ~19% positive rate (Eczema)
    }
    ...
    def evaluate(X, y, name):
        ...
        return dict(acc=acc, tn=tn, fp=fp, fn=fn, tp=tp, precision=precision, recall=recall, f1=f1, auc=auc)
    ...
    model.save_model(str(Path(r"C:\Users\tishy\Documents\Honors\models\curated_lightgbm.txt")))
    print("\nModel saved to models/curated_lightgbm.txt")
