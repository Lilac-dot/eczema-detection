"""
Same 90 colour features as extract_color_features.py, computed on the auto-cropped
images (dataset/manifest_cropped.csv, see scripts/crop_images.py) instead of the
original full-frame images.

Writes dataset/color_features_cropped.csv (same schema as color_features.csv).
"""
import csv
...
MANIFEST = ROOT / "manifest_cropped.csv"
OUT_CSV = ROOT / "color_features_cropped.csv"
RESIZE_TO = (128, 128)
...
def main():
    ...
    print(f"Extracting {len(names)} colour features from {len(reader)} cropped images...")
