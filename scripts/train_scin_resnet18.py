"""
ResNet18 trained from scratch (ImageNet-pretrained init) on SCIN-train only, for the
...
SCIN-train is small (488 images, balanced 244/244 by construction -- see
scripts/split_external_manifest.py) relative to the internal training set (2,327), so
...
from paths import ROOT, MODELS_DIR, SCIN_DIR
...
def main():
    ...
    train_ds = ManifestDataset(SCIN_DIR / "manifest_scin_train.csv", train_tf)
    val_ds = ManifestDataset(SCIN_DIR / "manifest_scin_val.csv", eval_tf)
    print(f"SCIN-train: {len(train_ds)}  SCIN-val: {len(val_ds)}")
    ...
    best_val_acc = 0.0
    out_path = MODELS_DIR / "scin_resnet18.pt"
